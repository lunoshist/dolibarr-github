...
class InterfaceAffaireTriggers extends DolibarrTriggers
{
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		switch ($action) {
			...
			//case 'BILL_DELETE':
			case 'BILL_PAYED':
				if (getDolGlobalInt('CLIENT_INVOICE_PAID_STATUS')) {
					dol_syslog("Trigger '".$this->name."' for action '$action' launched by ".__FILE__.". id=".$object->id);			
					
					$statusPaid = getDolGlobalInt('CLIENT_INVOICE_PAID_STATUS');
		
					// 1 - Mettre à jour le status de la facture
					$object->oldcopy = dol_clone($object, 2);
		
					$object->array_options["options_aff_status"] = $statusPaid;
					$result = $object->updateExtraField('aff_status', 'FACTURE_MODIFY');
					if ($result < 0) {
					...
					// Ne pas mettre l'étape à terminé si il reste des factures en cours
					$status = $affaire->getStatus('', $statusPaid);
					$array_of_fact = checkFactureExist($affaire);
					if ($status->fk_type >= 300 && is_array($array_of_fact)) {
