...
/**
 *   	\file       affaire_list.php
 *		\ingroup    affaire
 *		\brief      List page for affaire
 */
...
require_once __DIR__.'/class/affaire.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('affaire', 'affaire', 'write');
	$permissiontodelete = $user->hasRight('affaire', 'affaire', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("affaire")) {
	accessforbidden('Module affaire not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Affaire';
	$objectlabel = 'Affaire';
	$uploaddir = $conf->affaire->dir_output;
...
$title = $langs->trans("Affaires");
//$help_url = "EN:Module_Affaire|FR:Module_Affaire_FR|ES:Módulo_Affaire";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/affaire/affaire_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-affaire page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
