...
$title = $langs->trans('SUIVI DES PROPALS');
		...
		foreach ($array_affaires as $affaireID) {
			...
			foreach ($array_of_propal as $propalID) {
				...
				print '<tr class="'.($pair ? 'pair' : 'impair').'">';
					print '<td></td>';
					print '<td align="left"><a href='.$path.'</a><span class="opacitymedium"> - '.(isset($propal->ref_customer) ? $propal->ref_customer : $propal->ref_client)."</span></td>";
...
function fetchAffaire() {
	...
	foreach($workflows as $workflow) {
		$steplabel = empty(getDolGlobalString('STEP_PROPAL_FOR_WORKFLOW_'.$workflow->rowid)) ? 'propal' : getDolGlobalString('STEP_PROPAL_FOR_WORKFLOW_'.$workflow->rowid);
		$step = fetchStep(0, $steplabel, $workflow);
		$sql = 'SELECT rowid, fk_status FROM '.MAIN_DB_PREFIX.'affaire_affaire WHERE fk_workflow_type='.$workflow->rowid.' AND fk_step='.$step->rowid;

		$resql = $db->query($sql);
		if ($resql) {
			while ($affaire = $db->fetch_object($resql)) {
				if (!isset($array_of_affaire[$affaire->fk_status])) {
					$array_of_affaire[$affaire->fk_status] = array();
				}
				$array_of_affaire[$affaire->fk_status][] = $affaire->rowid; 
			}
		} else {
			dol_print_error($db);
		}
