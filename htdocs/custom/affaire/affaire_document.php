...
/**
 *  \file       affaire_document.php
 *  \ingroup    affaire
 *  \brief      Tab for documents linked to Affaire
 */
...
$langs->loadLangs(array("affaire@affaire", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->affaire->multidir_output[$object->entity ? $object->entity : $conf->entity]."/affaire/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('affaire', 'affaire', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Affaire")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-affaire page-card_document');

// Show tabs
$head = affairePrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Affaire"), -1, $object->picto);
...
$modulepart = 'affaire';
$param = '&id='.$object->id;
//$relativepathwithnofile='affaire/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'affaire/'.dol_sanitizeFileName($object->ref).'/';
