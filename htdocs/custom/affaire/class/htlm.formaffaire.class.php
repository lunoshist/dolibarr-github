...
/**
 *      \file       affaire/class/html.formaffaire.class.php
 *      \ingroup    core
 *      \brief      Class file for html component affaire
 */
...
class FormAffaires extends Form
{
	...
	/**
	 * Output a combo list with affaires qualified for a third party / user
	 *
	 * @param int 		$socid 			Id third party (-1=all, 0=only affaires not linked to a third party, id=affaires not linked or linked to third party id)
	 * @param string|Affaire $selected  Id of preselected affaire or Affaire (or ''). Note: If you know the ref, you can also provide it into $selected_input_value to save one request in some cases.
	 * @param string 	$htmlname 		Name of HTML field
	 * @param int 		$maxlength 		Maximum length of label
	 * @param int 		$option_only 	Return only html options lines without the select tag
	 * @param int|string	$show_empty 	Add an empty line
	 * @param int 		$discard_closed Discard closed affaires (0=Keep, 1=hide completely, 2=Disable). Use a negative value to not show the "discarded" tooltip.
	 * @param int 		$forcefocus 	Force focus on field (works with javascript only)
	 * @param int 		$disabled 		Disabled
	 * @param int 		$mode 			0 for HTML mode and 1 for JSON mode
	 * @param string 	$filterkey 		Key to filter on ref or title
	 * @param int 		$nooutput 		No print output. Return it only.
	 * @param int 		$forceaddid 	Force to add affaire id in list, event if not qualified
	 ...
	 */
	public function select_affaires($socid = -1, $selected = '', $htmlname = 'affaireid', $maxlength = 16, $option_only = 0, $show_empty = 1, $discard_closed = 0, $forcefocus = 0, $disabled = 0, $mode = 0, $filterkey = '', $nooutput = 0, $forceaddid = 0, $morecss = '', $htmlid = '', $morefilter = '')
	{
		// phpcs:enable
		...
		if (!empty($conf->use_javascript_ajax) && getDolGlobalString('AFFAIRE_USE_SEARCH_TO_SELECT')) {
			$placeholder = '';

			if ($selected && empty($selected_input_value)) {
				require_once dol_buildpath('/affaire/class/affaire.class.php');
				$affaire = new Affaire($this->db);
				$affaire->fetch($selected);
				$selected_input_value = $affaire->ref;
			}
			...
			$out .= ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT . '/affaire/ajax/affaires.php', $urloption, $conf->global->AFFAIRE_USE_SEARCH_TO_SELECT, 0, array());
		} else {
			$out .= $this->select_affaires_list($socid, $selected, $htmlname, $maxlength, $option_only, $show_empty, abs($discard_closed), $forcefocus, $disabled, 0, $filterkey, 1, $forceaddid, $htmlid, $morecss, $morefilter);
		}
		if ($discard_closed > 0) {
			if (!empty($form)) {
				$out .= $form->textwithpicto('', $langs->trans("ClosedAffairesAreHidden"));
			}
		}
	...
	/**
	 * Returns an array with affaires qualified for a third party
	 *
	 * @param int 		$socid 			Id third party (-1=all, 0=only affaires not linked to a third party, id=affaires not linked or linked to third party id)
	 * @param int 		$selected 		Id affaire preselected
	 * @param string 	$htmlname 		Name of html component
	 * @param int 		$maxlength 		Maximum length of label
	 * @param int 		$option_only 	Return only html options lines without the select tag
	 * @param int|string	$show_empty Add an empty line
	 * @param int 		$discard_closed Discard closed affaires (0=Keep,1=hide completely,2=Disable)
	 * @param int 		$forcefocus 	Force focus on field (works with javascript only)
	 * @param int 		$disabled 		Disabled
	 ...
	 */
	public function select_affaires_list($socid = -1, $selected = 0, $htmlname = 'affaireid', $maxlength = 24, $option_only = 0, $show_empty = 1, $discard_closed = 0, $forcefocus = 0, $disabled = 0, $mode = 0, $filterkey = '', $nooutput = 0, $forceaddid = 0, $htmlid = '', $morecss = 'maxwidth500', $morefilter = '')
	{
		// phpcs:enable
		...
		if (getDolGlobalString('AFFAIRE_HIDE_UNSELECTABLES')) {
			$hideunselectables = true;
		}
		if (getDolGlobalInt('AFFAIRE_ALWAYS_DISCARD_CLOSED_AFFAIRES_IN_SELECT')) {
			$discard_closed = 1;
		}

		$affairesListId = false;
		if (!$user->hasRight('affaire', 'all', 'lire')) {
			$affairestatic = new Affaire($this->db);
			$affairesListId = $affairestatic->getAffairesAuthorizedForUser($user, 0, 1);
		}

		// Search all affaires
		$sql = "SELECT p.rowid, p.ref, p.label, p.fk_soc, p.fk_status, p.public";
		$sql .= " FROM " . $this->db->prefix() . "affaire_affaire as p LEFT JOIN " . $this->db->prefix() . "societe as s ON s.rowid = p.fk_soc";
		// $sql .= " WHERE p.entity IN (" . getEntity('affaire') . ")";
		if ($affairesListId !== false) {
			$sql .= " AND p.rowid IN (" . $this->db->sanitize($affairesListId) . ")";
		}
		if ($socid == 0) {
			$sql .= " AND (p.fk_soc=0 OR p.fk_soc IS NULL)";
		}
		if ($socid > 0) {
			if (!getDolGlobalString('AFFAIRE_ALLOW_TO_LINK_FROM_OTHER_COMPANY')) {
				$sql .= " AND (p.fk_soc=" . ((int) $socid) . " OR p.fk_soc IS NULL)";
			} elseif (getDolGlobalString('AFFAIRE_ALLOW_TO_LINK_FROM_OTHER_COMPANY') != 'all') {    // AFFAIRE_ALLOW_TO_LINK_FROM_OTHER_COMPANY is 'all' or a list of ids separated by coma.
				$sql .= " AND (p.fk_soc IN (" . $this->db->sanitize(((int) $socid) . ", " . getDolGlobalString('AFFAIRE_ALLOW_TO_LINK_FROM_OTHER_COMPANY')) . ") OR p.fk_soc IS NULL)";
			}
		}
		if (!empty($filterkey)) {
			$sql .= natural_search(array('p.label', 'p.ref'), $filterkey);
		}
		if ($morefilter) {
		...
		if ($resql) {
			...
			if ($num) {
				while ($i < $num) {
					...
					// If we ask to filter on a company and user has no permission to see all companies and affaire is linked to another company, we hide affaire.
					if ($socid > 0 && (empty($obj->fk_soc) || $obj->fk_soc == $socid) && !$user->hasRight('societe', 'lire')) {
						// Do nothing
					} else {
						if ($discard_closed == 1 && $obj->fk_status == 2 && $obj->rowid != $selected) { // We discard closed except if selected
							$i++;
							continue;
						}

						$labeltoshow = dol_trunc($obj->ref, 18);
						//if ($obj->public) $labeltoshow.=' ('.$langs->trans("SharedAffaire").')';
						//else $labeltoshow.=' ('.$langs->trans("Private").')';
						$labeltoshow .= ', ' . dol_trunc($obj->label, $maxlength);

						$disabled = 0;
						if ($obj->fk_status == 0) {
							$disabled = 1;
							$labeltoshow .= ' - ' . $langs->trans("Draft");
						} elseif ($obj->fk_status == 2) {
							if ($discard_closed == 2) {
								$disabled = 1;
							}
							$labeltoshow .= ' - ' . $langs->trans("Closed");
						} elseif (!getDolGlobalString('AFFAIRE_ALLOW_TO_LINK_FROM_OTHER_COMPANY') && $socid > 0 && (!empty($obj->fk_soc) && $obj->fk_soc != $socid)) {
							$disabled = 1;
	 ...
	 * @param string $affairesListId ''=Automatic filter on affaire allowed. List of id=Filter on affaire ids.
	 * @param string $showmore 'all' = Show affaire info, 'progress' = Show task progression, ''=Show nothing more
	 * @param User $usertofilter User object to use for filtering
	 * @param int 	$nooutput 		1=Return string, do not send to output
	 *
	 * @return int|string                   Nbr of tasks if OK, <0 if KO. If nooutput=1: Return a HTML select string.
	 */
	public function selectTasks($socid = -1, $selected = 0, $htmlname = 'taskid', $maxlength = 24, $option_only = 0, $show_empty = '1', $discard_closed = 0, $forcefocus = 0, $disabled = 0, $morecss = 'maxwidth500', $affairesListId = '', $showmore = 'all', $usertofilter = null, $nooutput = 0)
	{
		global $user, $conf, $langs;
		...
		if (empty($affairesListId)) {
			if (!$usertofilter->hasRight('affaire', 'all', 'lire')) {
				$affairestatic = new Affaire($this->db);
				$affairesListId = $affairestatic->getAffairesAuthorizedForUser($usertofilter, 0, 1);
			}
		}

		// Search all affaires
		$sql = "SELECT t.rowid, t.ref as tref, t.label as tlabel, t.progress,";
		$sql .= " p.rowid as pid, p.ref, p.title, p.fk_soc, p.fk_status, p.public, p.usage_task,";
		$sql .= " s.nom as name";
		$sql .= " FROM " . $this->db->prefix() . "affaire as p";
		$sql .= " LEFT JOIN " . $this->db->prefix() . "societe as s ON s.rowid = p.fk_soc,";
		$sql .= " " . $this->db->prefix() . "affaire_task as t";
		$sql .= " WHERE p.entity IN (" . getEntity('affaire') . ")";
		$sql .= " AND t.fk_affaire = p.rowid";
		if ($affairesListId) {
			$sql .= " AND p.rowid IN (" . $this->db->sanitize($affairesListId) . ")";
		}
		if ($socid == 0) {
			...
			if ($num) {
				while ($i < $num) {
					...
					if ($socid > 0 && (empty($obj->fk_soc) || $obj->fk_soc == $socid) && !$usertofilter->hasRight('societe', 'lire')) {
						...
						if ($discard_closed == 1 && $obj->fk_status == Affaire::STATUS_CLOSED) {
							$i++;
							continue;
						}
						...
						$disabled = 0;
						if ($obj->fk_status == Affaire::STATUS_DRAFT) {
							$disabled = 1;
						} elseif ($obj->fk_status == Affaire::STATUS_CLOSED) {
							if ($discard_closed == 2) {
								$disabled = 1;
							}
						...
						if (preg_match('/all/', $showmore)) {
							$labeltoshow .= dol_trunc($obj->ref, 18); // Affaire ref
							//if ($obj->public) $labeltoshow.=' ('.$langs->trans("SharedAffaire").')';
							//else $labeltoshow.=' ('.$langs->trans("Private").')';
	...
	/**
	 *    Build a HTML select list of element of same thirdparty to suggest to link them to affaire
	 *
	 * @param string $table_element Table of the element to update
	 * @param int|string $socid If of thirdparty to use as filter or 'id1,id2,...'
	 * @param string $morecss More CSS
	 * @param int $limitonstatus Add filters to limit length of list to opened status (for example to avoid ERR_RESPONSE_HEADERS_TOO_BIG on affaire/element.php page). TODO To implement
	 * @param string $affairekey Equivalent key  to fk_affaire for actual table_element
	 * @param string $placeholder Placeholder
	 * @return    int|string                        The HTML select list of element or '' if nothing or -1 if KO
	 */
	public function select_element($table_element, $socid = 0, $morecss = '', $limitonstatus = -2, $affairekey = "fk_affaire", $placeholder = '')
	{
		// phpcs:enable
		global $conf, $langs;

		if ($table_element == 'affaire_task') {
			return ''; // Special cas of element we never link to a affaire (already always done)
		}
		...
		if (!in_array($table_element, array('expensereport_det', 'stock_mouvement'))) {
			$sql .= ' AND t.entity IN (' . getEntity('affaire') . ')';
		}
		if ($linkedtothirdparty) {
	 ...
	 *  @return	void
	 */
	public function selectAffairesStatus($selected = '', $short = 0, $hmlname = 'order_status')
	{
		$options = array();
		...
		$tmpaffaire = new Affaire($this->db);

		foreach ($statustohow as $key => $value) {
			$tmpaffaire->statut = $key;
			$options[$value] = $tmpaffaire->getLibStatut($short);
		}
	...
	/**
	 *  Output a combo list with invoices and lines qualified for a affaire
	 *
	...
	public function selectInvoiceAndLine($selectedInvoiceId = 0, $selectedLineId = 0, $htmlNameInvoice = 'invoiceid', $htmlNameInvoiceLine = 'invoicelineid', $morecss = 'maxwidth500', $filters = array(), $lineOnly = 0)
	{
		...
		if (empty($lineOnly)) {
			...
			$sql .= ' INNER JOIN ' . $this->db->prefix() . 'facture as f ON f.fk_affaire = p.rowid';
			$sql .= " WHERE p.entity IN (" . getEntity('affaire') . ")";
			if (!empty($filters)) {
