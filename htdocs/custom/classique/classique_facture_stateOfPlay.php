...
/**
 * \file 	htdocs/custom/classique/classique_facture_stateOfPlay.php
 * \ingroup affaire
...
if (empty($reshook)) {
	...
	if ($id && $action == 'changeStatus') {
		...
		if (empty($newStatus)) $newStatus = GETPOST('newStatus', 'aZ09');
...
if (getDolGlobalInt('DEBUG')) {
	print implode("\n", $INFO)."<br><br>";
	print dol_workflow_tabs($affaire, $thisStepName, $workflow);
} else {
...
if ($action == 'create') {
	$fact = checkFactureExist($affaire);
	if (!empty($fact)) {
		if (is_array($fact)) {
			setEventMessages("Plusieurs factures exitent déjà !!!", null, 'warnings');
		...
		if ($fact > 0) {
			setEventMessages('Une facture existe déjà', null, 'warnings');
