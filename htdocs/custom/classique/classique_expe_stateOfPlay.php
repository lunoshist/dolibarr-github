		...
		if ($status_for == 'both' || $status_for == 'object') {
			...
			if ($resql) {
				...
				if (!getDolGlobalString('GLOBAL_CHANGE_STATUS_WITHOUT_CHANGING_SYSTEM_STATUS') && !getDolGlobalString('EXPEDITION_CHANGE_STATUS_WITHOUT_CHANGING_SYSTEM_STATUS')) {
					...
					// 		// REOPEN
					// 		if (((getDolGlobalString('EXPEDITION_REOPEN_UNSIGNED_ONLY') && $object->statut == Expedition::STATUS_NOTSIGNED) || (!getDolGlobalString('EXPEDITION_REOPEN_UNSIGNED_ONLY') && ($object->statut == Expedition::STATUS_SIGNED || $object->statut == Expedition::STATUS_NOTSIGNED || $object->statut == Expedition::STATUS_BILLED || $object->statut == Expedition::STATUS_CANCELED))) && $usercanclose) {
					// 			// prevent browser refresh from reopening proposal several times
					// 			if ($object->statut == Expedition::STATUS_SIGNED || $object->statut == Expedition::STATUS_NOTSIGNED || $object->statut == Expedition::STATUS_BILLED || $object->statut == Expedition::STATUS_CANCELED) {
					// 				$db->begin();
	
					// 				$newstatus = (getDolGlobalInt('EXPEDITION_SKIP_ACCEPT_REFUSE') ? Expedition::STATUS_DRAFT : Expedition::STATUS_VALIDATED);
					...
					// 		}
					// 		// VALIDATED
					// 		else if ((($object->statut == Expedition::STATUS_DRAFT && $object->total_ttc >= 0 && count($object->lines) > 0)
					// 		|| ($object->statut == Expedition::STATUS_DRAFT && getDolGlobalString('EXPEDITION_ENABLE_NEGATIVE') && count($object->lines) > 0)) && $usercanvalidate) {
					// 			$error = 0;
					...
					// 			if ($result > 0 && getDolGlobalString('EXPEDITION_SKIP_ACCEPT_REFUSE')) {
					...
					// 			$error = "Impossible mettre le status à $obj->label (System status : Expedition::STATUS_VALIDATED) car elle ne contient pas de ligne ";
					// 		} else if ($object->statut != Expedition::STATUS_VALIDATED) {
					// 			$error = "Impossible mettre le status à $obj->label (System status : Expedition::STATUS_VALIDATED) depuis l'ancien status system : ".$object->LibStatut($object->statut);
					// 		}
					// 		break;
					// 	case (200 <= $code && $code <= 299):
					// 		// SIGNED
					// 		// prevent browser refresh from closing proposal several times
					// 		if ($object->statut == $object::STATUS_VALIDATED || $object->statut == $object::STATUS_NOTSIGNED || (getDolGlobalString('EXPEDITION_SKIP_ACCEPT_REFUSE') && $object->statut == $object::STATUS_DRAFT)) {
					// 			$db->begin();
				
					// 			$result = $object->closeProposal($user, Expedition::STATUS_SIGNED);
					// 			if ($result < 0) {
					// 				setEventMessages($object->error, $object->errors, 'errors');
					// 				$error++;
					// 			}
					...
					// 				$action = '';
					// 			}
					// 		} else if ($object->statut != Expedition::STATUS_SIGNED) {
					// 			$error = "Impossible mettre le status à $obj->label (System status : Expedition::STATUS_SIGNED) depuis l'ancien status system : ".$object->LibStatut($object->statut);
					// 		}
					// 		break;
					...
					// 		// NOTSIGNED
					// 		// prevent browser refresh from closing proposal several times
					// 		if ($object->statut == $object::STATUS_VALIDATED || (getDolGlobalString('EXPEDITION_SKIP_ACCEPT_REFUSE') && $object->statut == $object::STATUS_DRAFT)) {
					// 			$db->begin();
				
					// 			$result = $object->closeProposal($user, Expedition::STATUS_NOTSIGNED);
					// 			if ($result < 0) {
					// 				setEventMessages($object->error, $object->errors, 'errors');
					// 				$error++;
					// 			}
					...
					// 				$action = '';
					// 			}
					// 		} else if ($object->statut != Expedition::STATUS_VALIDATED) if ($object->statut != Expedition::STATUS_NOTSIGNED) {
					// 			$error = "Impossible mettre le status à $obj->label (System status : Expedition::STATUS_NOTSIGNED) depuis l'ancien status system : ".$object->LibStatut($object->statut);
					// 		}
					// 		break;
					// }
				}
